...
def main():
    ...
    if api_token and project_id:
        print("[OK] Evidently Cloud credentials found")
    else:
        print("[WARN] Evidently Cloud credentials not found.")
    ...
    test_suite = results.get("tests")
    if test_suite is not None:
        # Check if the test suite passed
        test_results = test_suite.as_dict()
        passed = all(test.get("status") == "SUCCESS" for test in test_results.get("tests", []))
        
        if not passed:
            print("\n[ALERT]: Data Drift or Quality Issues Detected!")
            
            # Trigger Alert
            alerter = AlertManager()
            drift_share = 0 # Default/placeholder
            
            # Try to extract details if possible (simplified for now)
            failed_tests = [t.get("name") for t in test_results.get("tests", []) if t.get("status") != "SUCCESS"]
            
            alerter.send_alert(
                title="Data Monitoring Failed",
                message=f"The monitoring test suite failed. Issues detected in: {', '.join(failed_tests)}",
                level="ERROR",
                details={"failed_tests": failed_tests, "report_path": f"{args.output_dir}/test_suite_report.html"}
            )
            # Exit with error code to signal pipeline failure
            sys.exit(1) 
